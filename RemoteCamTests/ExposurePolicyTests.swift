...
final class ExposurePolicyTests: XCTestCase {
    ...
    func testAutoClampsTheBiasIntoTheDeviceRange() {
        XCTAssertEqual(ExposurePolicy.resolve(.auto(bias: 0), facts: phone, isRecording: false), .auto(bias: 0))
        XCTAssertEqual(ExposurePolicy.resolve(.auto(bias: 1.5), facts: phone, isRecording: true), .auto(bias: 1.5))
        XCTAssertEqual(ExposurePolicy.resolve(.auto(bias: 20), facts: phone, isRecording: false), .auto(bias: 8))
        XCTAssertEqual(ExposurePolicy.resolve(.auto(bias: -20), facts: phone, isRecording: false), .auto(bias: -8))
        // Auto never depends on custom-exposure support.
        var noCustom = phone
        noCustom.supportsCustom = false
        XCTAssertEqual(ExposurePolicy.resolve(.auto(bias: 1), facts: noCustom, isRecording: false), .auto(bias: 1))
    }
    ...
    func testInRangeValuesPassThrough() {
        let plan = ExposurePolicy.resolve(.manual(durationSeconds: 1.0 / 250, iso: 400),
                                          facts: phone, isRecording: false)
        XCTAssertEqual(plan, .manual(durationSeconds: 1.0 / 250, iso: 400))
    }

    func testValuesClampIntoFormatRange() {
        let tooLong = ExposurePolicy.resolve(.manual(durationSeconds: 30, iso: 1_000_000),
                                             facts: phone, isRecording: false)
    ...
    func testZeroKeepsCurrentValue() {
        ...
        let shutterOnly = ExposurePolicy.resolve(.manual(durationSeconds: 0.5, iso: 0),
                                                 facts: phone, isRecording: false)
